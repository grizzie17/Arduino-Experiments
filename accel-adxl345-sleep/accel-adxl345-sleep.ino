//
// test routines to put ADXL345 and the MCU into
// low power mode.  Then we need to wakup the system
// if the ADXL345 generates an Activity interrupt.
//
...
const uint8_t k_maskInterrupt = k_maskActivity | ADXL_M_INACTIVITY;
...
enterSleep()
{
    ...
    g_tSleep.sleepPinInterrupt( k_pinINT, RISING );
...
loop()
{
    ...
    {
        if ( 1000 * 3 < abs( g_uTimeCurrent - g_uTimeInterrupt ) )
        {
            digitalWrite( k_pinLED, LOW );
            g_nActivity = 0;
        }
